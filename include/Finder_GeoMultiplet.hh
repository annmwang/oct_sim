...
// added into make this code work
int XROAD = 8; // size of x road in strips
double B = (1/TMath::Tan(1.5/180.*TMath::Pi()));
int ylow = 0.0;
int yhigh = 2200.0;
int UVFACTOR = round((yhigh-ylow)/(B * 0.4 * 2)/XROAD);
int bc_wind = 8;
int NSTRIPS_UP_UV = 4;
int NSTRIPS_DN_UV = 0;
int NSTRIPS_UP_XX = 4;
      ...
      //if (roads[i]->Coincidence(bc_wind)){
        if (db){
          ...
          for (unsigned int k = 0; k < roads[i]->Hits().size(); k++){
            printf("Hit (layer, strip, art bcid): (%d,%4.4f,%d)", roads[i]->Hits()[k].MMFE8Index(),roads[i]->Hits()[k].Channel(),roads[i]->Hits()[k].BC());
            cout << endl;
          }
        }
        ntrigs++;
