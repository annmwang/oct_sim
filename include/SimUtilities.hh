...
std::vector<int> kill_random_64PCBS(
    int killran, // bool if you want to kill one plane PCB randomly
...
//  - number of PCBs per plane
// Output: no ouput, operates on the detector hit vector 
std::vector<int> kill_random_LEGACY(
    int killran, // bool if you want to kill one plane PCB randomly
    int killxran, // bool if you want to kill one X plane PCB randomly
    int killuvran, // bool if you want to kill one UV plane PCB randomly
    int NPLANES, // number of planes on the detector
...
std::tuple< std::vector<int>, std::vector<Hit*>, int, int, int, int > get_hits_64PCBs(
    int NPLANES, // number of planes on the detector
    int NPCB_PER_PLANE, // number of PCBs per plane
...
//  - number of hits on x1, u, v, x2 planes as determined by hits in the PCBs
//  - fills the x strip resolution histogram
std::tuple< std::vector<int>, std::vector<Hit*>, int, int, int, int > get_hits_LEGACY(
    int NPLANES, // number of planes on the detector
    GeoOctuplet *GEOMETRY, // geometry of the octuplet
    TH1F *h_xres_strip, // x plane resolution histogram
    TRandom3 *ran, // random number generator from main method
...
//  - number of hits on x1, u, v, x2 planes as determined by hits in the PCBs
//  - fills the x strip resolution histogram
std::tuple< std::vector<int>, std::vector<Hit*>, int, int, int, int > get_hits(
    int NPLANES, // number of planes on the detector
    int NPCB_PER_PLANE, // number of PCBs per plane
    GeoOctuplet *GEOMETRY, // geometry of the octuplet
    TH1F *h_xres_strip, // x plane resolution histogram
...
)
{
        ...
        if (legacy){
            ...
            printf("*****************************************************************\n");*/
            std::tie(art_bc,hits, n_x1, n_u, n_v, n_x2) = get_hits_LEGACY(NPLANES, GEOMETRY, h_xres_strip, ran, sig_art, m_sig_art_x, bc_length, xpos, ypos, smear_art, funcsmear_art, func, bkgonly, oct_hitmask);
            return std::make_tuple(art_bc, hits, n_x1, n_u, n_v, n_x2);
        }
        else{
            /*
            ...
            printf("*****************************************************************\n");*/
            std::tie(art_bc,hits, n_x1, n_u, n_v, n_x2) = get_hits_64PCBs(NPLANES, NPCB_PER_PLANE, GEOMETRY, h_xres_strip, ran, sig_art, m_sig_art_x, bc_length, xpos, ypos, smear_art, funcsmear_art, func, bkgonly, oct_hitmask);
            return std::make_tuple(art_bc, hits, n_x1, n_u, n_v, n_x2);
        }
